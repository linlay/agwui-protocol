...
完整细分图请见：[交互时序图](../guides/interaction-sequences.md)
...
- `05-agw-seq-question.svg`：question 分支，`questions` 在 `awaiting.payload`
- `06-agw-seq-approval.svg`：approval 分支，`questions` 在 `awaiting.ask` 顶层
- `07-agw-seq-artifact.svg`：一次工具结果，多条 `artifact.publish`
